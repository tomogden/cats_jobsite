...
class CATS_sidebar extends CATS_Controller
{
    public function init()
    {
        global $catsPageID;

        if ($this->_session->hasRecentJobs())
        {
            $this->_showRecentJobs();
        }

        /* Only display sidebar if we're not already viewing the jobs area */
        if (get_the_ID() != $catsPageID)
        {
            $this->_showTopJobs();
        }
        else if (CATS_Utility::getPost('cc', false) != 'onlogin')
        {
    ...
    private function _showRecentJobs()
    {
